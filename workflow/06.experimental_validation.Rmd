...
tip.use <- data.frame(Species=taxtree$tip.label) %>% 
  ...
  mutate(strain_use = if_else(Species %in% validation_strains, "Validation", "Training")) %>% 
...
```{r plot.tree}
p.tree <- ggtree(taxtree, branch.length = "none") %<+% tip.use %<+% tip.gram +
  geom_tippoint(aes(color=strain_use, shape=`Gram stain`)) +
  geom_tiplab(size=2.8, offset = .1)+
  hexpand(ratio=.15) +
  vexpand(ratio = .0000001) +
  xlim(-.1, 7) +
...
```{r save.tree}
ggsave(plot=p.tree, filename = file.path(OUTPUT_DIR, "taxtree.pdf"),
